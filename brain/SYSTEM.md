...
- Date and time: `date`. Calendar: `omarchy-assistant calendar-add "<title>" "<YYYY-MM-DDTHH:MM>"` creates a Google Calendar event plus a nudge (resolve weekdays with `date -d 'next tuesday' +%F`); it reports asynchronously, so say it is being added. Email is not available here; say so.
